...
class StixCyberObservable:
    ...
    def list(self, **kwargs):
        ...
        if get_all:
            ...
            while result["data"]["stixCyberObservables"]["pageInfo"][
                    "hasNextPage"]:
                after = result["data"]["stixCyberObservables"]["pageInfo"][
                    "endCursor"]
    ...
    def read(self, **kwargs):
        ...
        if id is not None:
            self.opencti.log("info",
                             "Reading StixCyberObservable {" + id + "}.")
            query = ("""
    ...
    def create(self, **kwargs):
        ...
        create_indicator = (observable_data["x_opencti_create_indicator"]
        ...
        if simple_observable_key is not None:
            ...
            type = (observable_data["type"].title()
                    if "type" in observable_data else None)
        if type is None:
            return
        if type.lower() == "file":
            type = "StixFile"
        ...
        x_opencti_score = (observable_data["x_opencti_score"]
                           if "x_opencti_score" in observable_data else
                           x_opencti_score)
        if simple_observable_description is not None:
            x_opencti_description = simple_observable_description
        ...
        if type is not None:
            self.opencti.log(
                "info",
                "Creating Stix-Cyber-Observable {" + type +
                "} with indicator at " + str(create_indicator) + ".",
            )
            input_variables = {
                "type": type,
                "stix_id": stix_id,
            ...
            if type == "Autonomous-System":
                input_variables["AutonomousSystem"] = {
                    "number":
                    observable_data["number"],
                    "name":
                    observable_data["name"]
                    if "name" in observable_data else None,
                    "rir":
                    observable_data["rir"]
                    if "rir" in observable_data else None,
                }
            elif type == "Directory":
                input_variables["Directory"] = {
                    "path":
                    observable_data["path"],
                    "path_enc":
                    observable_data["path_enc"]
                    if "path_enc" in observable_data else None,
                    "ctime":
                    observable_data["ctime"]
                    if "ctime" in observable_data else None,
                    "mtime":
                    observable_data["mtime"]
                    if "mtime" in observable_data else None,
                    "atime":
                    observable_data["atime"]
                    if "atime" in observable_data else None,
                }
                ...
                if attribute is not None:
                    input_variables["DomainName"][
                        attribute] = simple_observable_value
                ...
                input_variables["EmailAddr"] = {
                    ...
                    if "display_name" in observable_data else None,
                }
            elif type == "Email-Message":
                input_variables["EmailMessage"] = {
                    "is_multipart":
                    observable_data["is_multipart"]
                    if "is_multipart" in observable_data else None,
                    "attribute_date":
                    observable_data["attribute_date"]
                    if "date" in observable_data else None,
                    "message_id":
                    observable_data["message_id"]
                    if "message_id" in observable_data else None,
                    "subject":
                    observable_data["subject"]
                    if "subject" in observable_data else None,
                    "received_lines":
                    observable_data["received_lines"]
                    if "received_lines" in observable_data else None,
                    "body":
                    observable_data["body"]
                    if "body" in observable_data else None,
                }
                ...
                input_variables["EmailMimePartType"] = {
                    ...
                    if "content_type" in observable_data else None,
                    "content_disposition":
                    observable_data["content_disposition"]
                    if "content_disposition" in observable_data else None,
                }
            elif type == "Artifact":
                input_variables["Artifact"] = {
                    "hashes":
                    hashes if len(hashes) > 0 else None,
                    "mime_type":
                    observable_data["mime_type"]
                    if "mime_type" in observable_data else None,
                    "payload_bin":
                    observable_data["payload_bin"]
                    if "payload_bin" in observable_data else None,
                    "url":
                    observable_data["url"]
                    if "url" in observable_data else None,
                    "encryption_algorithm":
                    observable_data["encryption_algorithm"]
                    if "encryption_algorithm" in observable_data else None,
                    "decryption_key":
                    observable_data["decryption_key"]
                    if "decryption_key" in observable_data else None,
                }
            elif type == "StixFile":
                input_variables["StixFile"] = {
                    ...
                    hashes if len(hashes) > 0 else None,
                    "size":
                    observable_data["size"]
                    if "size" in observable_data else None,
                    "name":
                    observable_data["name"]
                    if "name" in observable_data else None,
                    "name_enc":
                    observable_data["name_enc"]
                    if "name_enc" in observable_data else None,
                    "magic_number_hex":
                    observable_data["magic_number_hex"]
                    if "magic_number_hex" in observable_data else None,
                    "mime_type":
                    observable_data["mime_type"]
                    if "mime_type" in observable_data else None,
                    "mtime":
                    ...
                    if "atime" in observable_data else None,
                    "x_opencti_additional_names":
                    observable_data["x_opencti_additional_names"] if
                    "x_opencti_additional_names" in observable_data else None,
                }
                ...
                input_variables["X509Certificate"] = {
                    ...
                    if "is_self_signed" in observable_data else False,
                    "version":
                    observable_data["version"]
                    if "version" in observable_data else None,
                    "serial_number":
                    observable_data["serial_number"]
                    if "serial_number" in observable_data else None,
                    "signature_algorithm":
                    observable_data["signature_algorithm"]
                    if "signature_algorithm" in observable_data else None,
                    "issuer":
                    observable_data["issuer"]
                    if "issuer" in observable_data else None,
                    "validity_not_before":
                    observable_data["validity_not_before"]
                    if "validity_not_before" in observable_data else None,
                    "validity_not_after":
                    observable_data["validity_not_after"]
                    if "validity_not_after" in observable_data else None,
                    "subject":
                    observable_data["subject"]
                    if "subject" in observable_data else None,
                    "subject_public_key_algorithm":
                    observable_data["subject_public_key_algorithm"]
                    if "subject_public_key_algorithm" in observable_data else
                    None,
                    "subject_public_key_modulus":
                    observable_data["subject_public_key_modulus"] if
                    "subject_public_key_modulus" in observable_data else None,
                    "subject_public_key_exponent":
                    observable_data["subject_public_key_exponent"] if
                    "subject_public_key_exponent" in observable_data else None,
                }
            elif type == "IPv4-Addr":
                input_variables["IPv4Addr"] = {
                    "value":
                    observable_data["value"]
                    if "value" in observable_data else None,
                }
            elif type == "IPv6-Addr":
                ...
                input_variables["NetworkTraffic"] = {
                    "start":
                    observable_data["start"]
                    if "start" in observable_data else None,
                    "end":
                    observable_data["end"]
                    if "end" in observable_data else None,
                    "is_active":
                    observable_data["is_active"]
                    if "is_active" in observable_data else None,
                    "src_port":
                    observable_data["src_port"]
                    if "src_port" in observable_data else None,
                    "dst_port":
                    observable_data["dst_port"]
                    if "dst_port" in observable_data else None,
                    "protocols":
                    observable_data["protocols"]
                    if "protocols" in observable_data else None,
                    "src_byte_count":
                    observable_data["src_byte_count"]
                    if "src_byte_count" in observable_data else None,
                    "dst_byte_count":
                    observable_data["dst_byte_count"]
                    if "dst_byte_count" in observable_data else None,
                    "src_packets":
                    observable_data["src_packets"]
                    if "src_packets" in observable_data else None,
                    "dst_packets":
                    observable_data["dst_packets"]
                    if "dst_packets" in observable_data else None,
                }
            elif type == "Process":
                input_variables["Process"] = {
                    "is_hidden":
                    observable_data["is_hidden"]
                    if "is_hidden" in observable_data else None,
                    "pid":
                    observable_data["pid"]
                    if "pid" in observable_data else None,
                    "created_time":
                    observable_data["created_time"]
                    if "created_time" in observable_data else None,
                    "cwd":
                    observable_data["cwd"]
                    if "cwd" in observable_data else None,
                    "command_line":
                    observable_data["command_line"]
                    if "command_line" in observable_data else None,
                    "environment_variables":
                    observable_data["environment_variables"]
                    if "environment_variables" in observable_data else None,
                }
            elif type == "Software":
                input_variables["Software"] = {
                    ...
                    "cpe":
                    observable_data["cpe"]
                    if "cpe" in observable_data else None,
                    "swid":
                    observable_data["swid"]
                    if "swid" in observable_data else None,
                    "languages":
                    observable_data["languages"]
                    if "languages" in observable_data else None,
                    "vendor":
                    observable_data["vendor"]
                    if "vendor" in observable_data else None,
                    "version":
                    observable_data["version"]
                    if "version" in observable_data else None,
                }
                ...
                input_variables["UserAccount"] = {
                    ...
                    if "user_id" in observable_data else None,
                    "credential":
                    observable_data["credential"]
                    if "credential" in observable_data else None,
                    "account_login":
                    observable_data["account_login"]
                    if "account_login" in observable_data else None,
                    "account_type":
                    observable_data["account_type"]
                    if "account_type" in observable_data else None,
                    "display_name":
                    observable_data["display_name"]
                    if "display_name" in observable_data else None,
                    "is_service_account":
                    observable_data["is_service_account"]
                    if "is_service_account" in observable_data else None,
                    "is_privileged":
                    observable_data["is_privileged"]
                    if "is_privileged" in observable_data else None,
                    "can_escalate_privs":
                    observable_data["can_escalate_privs"]
                    if "can_escalate_privs" in observable_data else None,
                    "is_disabled":
                    observable_data["is_disabled"]
                    if "is_disabled" in observable_data else None,
                    "account_created":
                    observable_data["account_created"]
                    if "account_created" in observable_data else None,
                    "account_expires":
                    observable_data["account_expires"]
                    if "account_expires" in observable_data else None,
                    "credential_last_changed":
                    observable_data["credential_last_changed"]
                    if "credential_last_changed" in observable_data else None,
                    "account_first_login":
                    observable_data["account_first_login"]
                    if "account_first_login" in observable_data else None,
                    "account_last_login":
                    observable_data["account_last_login"]
                    if "account_last_login" in observable_data else None,
                }
            elif type == "Windows-Registry-Key":
                input_variables["WindowsRegistryKey"] = {
                    "attribute_key":
                    observable_data["key"]
                    if "key" in observable_data else None,
                    "modified_time":
                    observable_data["modified_time"]
                    if "modified_time" in observable_data else None,
                    "number_of_subkeys":
                    observable_data["number_of_subkeys"]
                    if "number_of_subkeys" in observable_data else None,
                }
                ...
                input_variables["WindowsRegistryKeyValueType"] = {
                    ...
                    if "data" in observable_data else None,
                    "data_type":
                    observable_data["data_type"]
                    if "data_type" in observable_data else None,
                }
            elif type == "X509-V3-Extensions-Type":
                input_variables["X509V3ExtensionsType"] = {
                    "basic_constraints":
                    observable_data["basic_constraints"]
                    if "basic_constraints" in observable_data else None,
                    "name_constraints":
                    observable_data["name_constraints"]
                    if "name_constraints" in observable_data else None,
                    "policy_constraints":
                    observable_data["policy_constraints"]
                    if "policy_constraints" in observable_data else None,
                    "key_usage":
                    observable_data["key_usage"]
                    if "key_usage" in observable_data else None,
                    "extended_key_usage":
                    observable_data["extended_key_usage"]
                    if "extended_key_usage" in observable_data else None,
                    "subject_key_identifier":
                    observable_data["subject_key_identifier"]
                    if "subject_key_identifier" in observable_data else None,
                    "authority_key_identifier":
                    observable_data["authority_key_identifier"]
                    if "authority_key_identifier" in observable_data else None,
                    "subject_alternative_name":
                    observable_data["subject_alternative_name"]
                    if "subject_alternative_name" in observable_data else None,
                    "issuer_alternative_name":
                    observable_data["issuer_alternative_name"]
                    if "issuer_alternative_name" in observable_data else None,
                    "subject_directory_attributes":
                    observable_data["subject_directory_attributes"]
                    if "subject_directory_attributes" in observable_data else
                    None,
                    "crl_distribution_points":
                    observable_data["crl_distribution_points"]
                    if "crl_distribution_points" in observable_data else None,
                    "inhibit_any_policy":
                    observable_data["inhibit_any_policy"]
                    if "inhibit_any_policy" in observable_data else None,
                    "private_key_usage_period_not_before":
                    ...
                    if "certificate_policies" in observable_data else None,
                    "policy_mappings":
                    observable_data["policy_mappings"]
                    if "policy_mappings" in observable_data else None,
                }
    ...
    def update_created_by(self, **kwargs):
        ...
        if id is not None:
            self.opencti.log(
                "info",
                "Updating author of Stix-Cyber-Observable {" + id +
    ...
    def add_marking_definition(self, **kwargs):
        ...
        if id is not None and marking_definition_id is not None:
            ...
            if stix_cyber_observable is None:
                ...
                return False
            if marking_definition_id in stix_cyber_observable[
                    "objectMarkingIds"]:
                return True
            else:
                self.opencti.log(
    ...
    def add_label(self, **kwargs):
        ...
        if id is not None and label_id is not None:
            self.opencti.log(
                "info",
                "Adding label {" + label_id + "} to Stix-Cyber-Observable {" +
                id + "}",
            )
            query = """
               mutation StixCyberObservableAddRelation($id: ID!, $input: StixMetaRelationshipAddInput) {
    ...
    def add_external_reference(self, **kwargs):
        ...
        if id is not None and external_reference_id is not None:
            ...
            if stix_domain_object is None:
                ...
                return False
            if external_reference_id in stix_domain_object[
    ...
    def ask_for_enrichment(self, **kwargs) -> str:
        ...
        if id is None or connector_id is None:
            self.opencti.log("error",
                             "Missing parameters: id and connector_id")
            return ""

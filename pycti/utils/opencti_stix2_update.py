...
class OpenCTIStix2Update:
    ...
    def add_object_marking_refs(self,
                                entity_type,
                                id,
                                object_marking_refs,
                                version=2):
    ...
    def remove_object_marking_refs(self,
                                   entity_type,
                                   id,
                                   object_marking_refs,
                                   version=2):
    ...
    def add_kill_chain_phases(self,
                              entity_type,
                              id,
                              kill_chain_phases,
                              version=2):
    ...
    def add_labels(self, entity_type, id, labels, version=2):
        for label in labels:
            if version == 2:
                label = label["value"]
            if entity_type == "relationship":
                self.opencti.stix_core_relationship.add_label(id=id,
                                                              label_name=label)
            elif StixCyberObservableTypes.has_value(entity_type):
                self.opencti.stix_cyber_observable.add_label(id=id,
                                                             label_name=label)
            else:
                self.opencti.stix_domain_object.add_label(id=id,
                                                          label_name=label)
    ...
    def remove_labels(self, entity_type, id, labels, version=2):
        for label in labels:
            ...
            if entity_type == "relationship":
                ...
                self.opencti.stix_domain_object.remove_label(id=id,
                                                             label_name=label)

    def replace_created_by_ref(self,
                               entity_type,
                               id,
                               created_by_ref,
                               version=2):
        if version == 2:
            created_by_ref = (created_by_ref[0]["value"]
                              if created_by_ref is not None else None)
        if entity_type == "relationship":
    ...
    def update_attribute(self, entity_type, id, input):
        # Relations
        if entity_type == "relationship":
            self.opencti.stix_core_relationship.update_field(id=id,
                                                             input=input)
        elif entity_type == "sighting":
            self.opencti.stix_sighting_relationship.update_field(id=id,
                                                                 input=input)
        # Observables
        elif StixCyberObservableTypes.has_value(entity_type):
        ...
        try:
            ...
            if "add" in data["x_opencti_patch"]:
                for key in data["x_opencti_patch"]["add"].keys():
                    ...
                    values = list(
                        map(lambda x: x["value"] if "value" in x else x, val))
                    inputs.append({
                        "key": key,
                        "value": values,
                        "operation": "add"
                    ...
                    if (
                        ...
                        if type(current_val) is list:
                            values = list(
                                map(
                                    lambda x: x["value"]
                                    if (type(current_val) is dict and "value"
                                        in x) else x,
                                    str(current_val),
                                ))
                            inputs.append({"key": key, "value": values})
                        else:
                            values = (current_val["value"] if
                                      (type(current_val) is dict
                                       and "value" in current_val) else
                                      str(current_val))
                            inputs.append({"key": key, "value": values})
